...
class AdminController extends Controller
{
    ...
    public function Addstudent(Request $req)
    {
        ...
        $validator = Validator::make($req->all(), [
            'name' => 'required',
            'student_id' => 'numeric|required|unique:users|min:4|max:12',
            'admin_id' => 'required',
    ...
    public function Addsubject(Request $req)
    {
        ...
        if ($validator->fails()) {
            ...
            $examsubject->subject_id = md5(microtime());
            $examsubject->subject_level = $req->subject_level;
            $examsubject->subject_type = $req->subject_type;
            $examsubject->pass_mark = $req->pass_mark;
    ...
    public function addquest($examcode, $title, $tname, $cat, $time)
    {

        //    dump($id);
        $question = DB::table('exam_question')->orderBy('created_at', 'ASC')->where('examcode', $examcode)//where('admin_id',Auth::user()->id && 'examcode',$id);
        //    ->limit(500)
        ->get();

        //get exam for particular examcode
        $id= Aexam::where('examcode',$examcode)->pluck('id')->first();


        $subject = DB::table('exam_subject')->orderBy('created_at', 'DESC')->where('examcode', $examcode)//where('admin_id',Auth::user()->id && 'examcode',$id);
        //    ->limit(500)
        ->get();
        $category = category::all();


        $exam_publish = DB::table('exam')->orderBy('created_at', 'DESC')->where(['admin_id' => Auth::user()->id, 'examcode' => $examcode])
            //->limit(50)
            ->get();

        //    dump($exam_publish);
        return view('addquestion', compact('exam_publish', 'question', 'category', 'subject', 'id', 'title', 'tname', 'cat', 'time','examcode'));
    }
...

    {
        ...
        $user->allow_exam_review = !$user->allow_exam_review;
        $user->save();
    ...
    public function DeleteExam(Request $req)
    {
        ...
        /* $exam= DB::table('exam')
             ->join('exam_subject','exam_subject.exam_code','=','exam.examcode')
             ->join('exam_question','exam_question.exam_code','=','exam.examcode')
             ->join('exam_question')*/
         $exam = Aexam::find($req->examcode);


        // $question= Addquestion::where('examcode', $req->examcode)->get();
        // $subject = examsubject::where('examcode', $req->examcode)->get();
        /* $question = Addquestion::where('examcode', $req->examcode)->get();
         $subject = examsubject::where('examcode', $req->examcode)->get();
         $question->delete();
         $subject->delete();*/
       /* $exam = DB::table('exam')
            ...
            ->join('exam_question', 'exam_question.examcode', '=', 'exam.examcode')
            ->select('exam.*', 'exam_question.*', 'exam_subject.*')
            ->where(['exam.examcode' => $req->examcode])
            ->get();*/
        $exam->delete();
       // $subject->delete();
       // $question->delete();
        return redirect()->back()->with('success', 'exam was deleted succesfully');
